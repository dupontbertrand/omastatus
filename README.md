...
Resource use is bounded before untrusted local data is decoded: JSON configuration/state files must be regular files owned by the current user and are opened without following symlinks or waiting on special files. They are limited to 1 MiB, with at most 128 categories and 256 services. Output captured from `ping`, `systemctl`, `docker`, or `kubectl` is limited to 256 KiB in total per check; a command is stopped on overflow or timeout before its output is parsed.
